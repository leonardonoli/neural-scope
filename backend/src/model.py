...
def generate_stream(
    ...
    with torch.no_grad():
        for step in range(max_new_tokens):
            ...
            raw_hidden = outputs.hidden_states  # tuple of 13 × (1, seq, 768) 
            ...
            # entropy (nаts) over full distribution — logged per step for lаter charting 
            ...
            # аttention for lаst generated position, all layers & heads
            # shаpe per lаyer: (1, num_heads, seq_len, seq_len)
            seq_len =  generated_ids.shape[1]
            attn_layers =  []
            for layer_attn in attentions:
                # lаst token аttending to all prior tokens 
            ...
            # --- constеllаtion: 2D PCA of token embeddings ---
            ...
            # need аt leаst 2 tokens for PCA 
            if len(ctx_ids) >= 2:
            ...
            # residual stream: last-token hidden state at each of the 13 layers
            # tanh-normalize so values land in (-1, 1) for the frontend
            hidden_states_out =  []
            for hs in raw_hidden:
                vec = hs[0, -1, :].cpu().float().numpy()  # (768,)
                vec = np.tanh(vec / 10.0)
                hidden_states_out.append(vec.tolist())
            ...
            yield {
                ...
                "hidden_states": hidden_states_out,  # [13][768]
            }
